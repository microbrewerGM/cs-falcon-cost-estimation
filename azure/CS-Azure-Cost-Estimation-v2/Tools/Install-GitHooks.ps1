...
.DESCRIPTION
This script installs a pre-commit hook that validates PowerShell syntax before 
allowing a commit to proceed. This helps prevent syntax errors from being 
...
$preCommitContent = @"
#!/bin/sh
# Pre-commit hook to validate PowerShell syntax
...
if [ -n "\$STAGED_PS_FILES" ]; then
  echo "Running PowerShell syntax validation on staged files..."
...
Write-Host "PowerShell syntax will now be validated before each commit." -ForegroundColor Cyan
