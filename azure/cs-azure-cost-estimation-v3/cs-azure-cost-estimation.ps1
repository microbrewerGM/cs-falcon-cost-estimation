...
function Start-CostEstimation {
    ...
    $pricingInfo = if ($ForceRefreshPricing) {
    ...
    $estimates = Get-AllSubscriptionsCostEstimate -CollectedData $collectedData -Pricing $pricingInfo
    ...
    $buSummary = Get-BusinessUnitCostSummary -SubscriptionEstimates $estimates
    ...
    $jsonSuccess = Export-SummaryToJson -SubscriptionEstimates $estimates -BusinessUnitSummary $buSummary -EntraIdData $collectedData.EntraIdMetrics -OutputFilePath $paths.SummaryJsonPath
    ...
    # Output summary
    $totalCost = ($estimates | Measure-Object -Property MonthlyCost -Sum).Sum
    $totalSubscriptions = $estimates.Count
    ...
    Write-Host "Total Estimated Annual Cost:  `$$($totalCost * 12).ToString('N2'))" -ForegroundColor Yellow
