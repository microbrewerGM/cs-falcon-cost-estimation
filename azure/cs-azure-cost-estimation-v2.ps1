...
function Get-PricingForRegion {
    ...
    if ($eventHubItem) {
        ...
        $formattedPricing.EventHubTU = $script:StaticPricing.default.EventHubTU # Fallback
    ...
    if ($storageItem) {
        ...
        $formattedPricing.StorageGB = $script:StaticPricing.default.StorageGB # Fallback
    ...
    if ($functionItem) {
        ...
        $formattedPricing.FunctionAppP0V3 = $script:StaticPricing.default.FunctionAppP0V3 # Fallback
    ...
    if ($keyVaultItem) {
        # Convert to cost per 10,000 operations
        $operationsMultiplier = 10000
        if ($keyVaultItem.unitOfMeasure -like "*10,000*") {
            $operationsMultiplier = 1
        }
        elseif ($keyVaultItem.unitOfMeasure -like "*100,000*") {
            $operationsMultiplier = 0.1
        }
        
        $formattedPricing.KeyVault = $keyVaultItem.retailPrice * $operationsMultiplier
        Write-Log "Key Vault price per 10,000 operations: $($formattedPricing.KeyVault)" -Level 'DEBUG' -Category 'Pricing'
    }
    else {
