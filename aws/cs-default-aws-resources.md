...
1.  **Asset Inventory (`asset_inventory.enabled`):** Allows CrowdStrike basic read access to discover and inventory AWS assets. (Core component)
2.  **Real-time Visibility / Indicator of Attack (IOA) (`realtime_visibility.enabled`):** Deploys infrastructure to stream AWS CloudTrail events to CrowdStrike for threat detection. (Core component)
3.  **Identity Protection (IDP) (`idp.enabled`):** (Optional) Enables integration features related to identity threat detection within AWS.
4.  **Sensor Management (`sensor_management.enabled`):** (Optional Feature - See details below) Facilitates deployment and management of Falcon sensors on EC2 instances.
5.  **Data Security Posture Management (DSPM) (`dspm.enabled`):** (Optional Feature - See details below) Deploys resources necessary for CrowdStrike to scan data stores.
...
## Core Resources Created by Scope

The specific AWS resources created depend heavily on the features enabled in the Terraform configuration (`enabled = true`) and the chosen scope (Organization vs. Individual Account). This section covers the core IOM/IOA components. Optional features (DSPM, Sensor Management) are detailed later.
...
* **IAM Role for Asset Inventory & Core Read Access:** Grants CrowdStrike necessary read-only permissions to assess resource configurations.
    ...
    * *Default/Common Name Concept:* `CrowdStrikeCSPMReader` (actual name based on TF output/config)
...
* **IAM Policy:** Attached to the core role, granting permissions required for enabled features (e.g., `ec2:DescribeInstances`, `s3:ListAllMyBuckets`, `iam:ListAccountAliases`, etc., for IOM/Asset Inventory and potentially IDP/Sensor Management read actions).
...
* **EventBridge (CloudWatch Events):**
    ...
    * **IAM Role for Event Forwarding:** Grants permissions for EventBridge/related services to process and forward events.
        * *Default/Common Name Concept:* `CrowdStrikeCSPMEventBridge`
...
### 3. Identity Protection (IDP) Resources (`idp.enabled = true`)
...
*(Note: The "Snapshot Scanning" components mentioned in some initial documentation, involving AWS Batch and specific EBS snapshot handling, are not explicitly detailed in the common Terraform resource/module documentation snippets (`crowdstrike_cloud_aws_account`, `CrowdStrike/cloud-registration/aws` module overview). They might be part of a different module, a more detailed aspect of DSPM not covered here, or configured separately. This document focuses on resources directly implied or confirmed by the primary Terraform details.)*

---

## Optional Feature: Data Security Posture Management (DSPM) (`dspm.enabled = true`)

These resources are deployed when DSPM is enabled in the Terraform configuration. They are often deployed regionally based on the `dspm_regions` list specified in the Terraform module.
...
* **Compute Resources (Likely EC2 or ECS/Fargate):** Resources launched temporarily to perform scanning tasks, assuming the `CrowdStrikeDSPMScannerRole`.
    * *Instance Type Example:* `c6a.2xlarge` (actual type may vary based on module version/defaults).
* **KMS Keys:** Potentially created for encrypting data during the scanning process.

---

## Optional Feature: Sensor Management (1-Click) (`sensor_management.enabled = true`)

These resources and configurations are related to enabling simplified Falcon sensor deployment via the CrowdStrike console.

* **IAM Permissions:** Enabling this likely adjusts IAM permissions within the core `CrowdStrikeCSPMReader` role or a dedicated role to allow Falcon to interact with services like AWS Systems Manager (SSM) for deploying/managing the Falcon sensor on EC2 instances. Requires specific API scopes (`CSPM sensor management`, `Installation tokens`, `Sensor download`).
* **AWS Systems Manager (SSM) Configurations:** May involve creating SSM Documents (e.g., for running deployment scripts) or other SSM configurations to facilitate sensor distribution and installation.
...
*(Based on typical configurations or details from initial documentation)*
...
* **Sensor Management (`sensor_management.enabled = true`):**
    * Primarily involves potential costs associated with AWS Systems Manager usage if exceeding free tier limits (e.g., SSM Automation runs).
